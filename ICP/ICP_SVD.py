...
def ICP_SVD(scan1, scan2, plotting=False, ignore=True):
    if ignore:
    ...
    def plot_correspondence(P, Q, correspondences, correp_lines=True):
        if correp_lines:
            for i in range(P.shape[0]):
    ...
    if plotting:
        correspondences = compute_correspondence(P, Q)[0]
    ...
    def SVD(Q, P, cross_covariance):
        # U, S, V = scipy.sparse.linalg.svds(cross_covariance, k=20, which='LM')
    ...
    def SVD_ICP(Q, P, max_iterations=10, plotting=False):
        loss = []
        center_of_P, P_centered = center_data(P)
        num_iterations = 0
        while True:
        ...
        # for i in range(num_iterations):
            ...
            if plotting: # and num_iterations % 10 == 0:
                plot_correspondence(P_centered, Q, correspondences)
                plt.title("Iteration: " + str(num_iterations))
                plt.pause(0.1)
                plt.clf()
            ...
            if num_iterations > 1:
                ...
                if diff < 0.00005:
                    if plotting: # and num_iterations % 10 == 0:
                        plot_correspondence(P_centered, Q, correspondences, correp_lines=False)
                        plt.title("Iteration: " + str(num_iterations))
                        plt.pause(0.5)
    ...
    R, t, loss = SVD_ICP(Q, P, max_iterations=100, plotting=plotting)
    ...
    if plotting:
        ...
        plt.legend()
        plt.show()
    t_vec = np.array([t[0], t[1], np.arccos(R[0,0])])
    return t_vec, loss, len(loss)
...
if __name__ == "__main__":
    ...
    # scans = np.load("/home/laser14/Documents/Personal/RPLidar/ICP/test_data/test_cb460_1.npy", allow_pickle=True)

    # Q = np.column_stack((scans[0][0].T,scans[0][1].T))
    # P = np.column_stack((scans[1][0].T,scans[1][1].T))

    Q = np.column_stack((points1[0].T,points1[1].T))
    P = np.column_stack((points2[0].T,points2[1].T))
    t, loss, num_iterations = ICP_SVD(Q, P, plotting=True, ignore=False)
